...
void handleClient(int clientSocket, int serverSocket, unordered_map<int, string> serverHash, int pnInt, const string serverKeysPath, const string serverPrvKeyPath, const string serverPubKeyPath, const string serverPath) {
    ...
    if (pingStr == "C") {
        ...
        cout << "SERVER HAS BEEN PINGED" << endl;
    }
}
...
int main() {
    ...
    while (true) {
        ...
        thread(handleClient, clientSocket, serverSocket, serverHash, pnInt, serverKeysPath, server_priv_path, server_pub_path, path).detach();
